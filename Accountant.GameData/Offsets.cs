...
public static class Signatures
{
    public const string GoldSaucerData =
        "?? 89 ?? ?? ?? 57 48 83 ?? ?? 48 8B ?? ?? ?? ?? ?? 48 33 ?? ?? 89 ?? ?? ?? 48 8B ?? ?? ?? ?? ?? 48 8B ?? E8 ?? ?? ?? ?? 48 8B ?? 4C ?? ?? ?? ?? ?? ?? ?? ?? ?? 48 8B ?? BA B5";
    ...
    public const string CustomDeliveryAllowances = "0F B6 41 24 4C 8B C1";

    public const string CustomDeliveryData       = "48 8D 0D ?? ?? ?? ?? 41 0F BA EC";
}
